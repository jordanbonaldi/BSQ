...
** Last update Sun Dec 18 13:00:15 2016 Bonaldi Jordan
*/
...
void	wait_connection(t_server *sv)
{
  ...
  if (!sv->nfd &&
      (sv->nfd = accept(sv->fd, (struct sockaddr *)&sv->server, (socklen_t *)&size)) < 0)
  ...
  if ((sv->nfd2 = accept(sv->fd, (struct sockaddr *)&sv->server, (socklen_t *)&size)) < 0)
    my_exit("ERROR: Impossible to obtain new Socket Descriptor :");
  else
